...
SoundEffect::SoundEffect(const int freq, int milliseconds)
{
    // Initialize the sound format we will request from sound card
    m_waveFormat.wFormatTag = WAVE_FORMAT_PCM;     // Uncompressed sound format
    m_waveFormat.nChannels = 1;                    // 1 = Mono, 2 = Stereo
    m_waveFormat.wBitsPerSample = 8;               // Bits per sample per channel
    //m_waveFormat.nSamplesPerSec = 11025;           // Sample Per Second
    m_waveFormat.nSamplesPerSec = 44100;           // Sample Per Second
    m_waveFormat.nBlockAlign = m_waveFormat.nChannels * m_waveFormat.wBitsPerSample / 8;
    m_waveFormat.nAvgBytesPerSec = m_waveFormat.nSamplesPerSec * m_waveFormat.nBlockAlign;
    m_waveFormat.cbSize = 0;

    m_bufferSize = 44100 * milliseconds / 1000;
    m_data = new char[m_bufferSize];

    // Allocate the array
    const auto period = m_waveFormat.nSamplesPerSec / freq;
    const auto halfPeriod = period / 2;

    for (size_t i = 0; i < m_bufferSize; ++i)
        //m_data[i] = ((i / (11025 / (2 * freq))) % 2) ? 255 : 0;
        m_data[i] = ((i / halfPeriod) % 2) ? 255 : 0;
}
...
void SoundEffect::Play()
{
    ...
    if (waveOutOpen(&m_waveOut, 0, &m_waveFormat, (DWORD_PTR)m_done, 0, CALLBACK_EVENT) != MMSYSERR_NOERROR)
    {
